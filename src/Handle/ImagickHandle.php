...
class ImagickHandle extends Handle
{
	...
	public function __construct(Captcha $captcha, string $image, array $config = [])
	{
        ...
        {
			throw new CaptchaException(captcha::lang('Need to support Imagick extension.'));
		}
	...
	public function getInfo(string $filePath = null): array
	{
        ...
        else
        {
			if (!is_file($filePath))
            {
				throw new CaptchaException(Captcha::lang('Image does not exist.'));
			}
        ...
        $range = ['image/jpeg', 'image/png', 'image/webp', 'image/x-webp'];
		if (!in_array($info['mime'], $range))
        {
			throw new CaptchaException(
                Captcha::lang('Please use jpeg and png or webp images.')
            );
		}
